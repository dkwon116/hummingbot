...
class UpbitWebSocketUnitTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.trading_pairs = ["BTC-KRW"]

    def setUp(self) -> None:
        super().setUp()
        self.log_records = []

        self.websocket = UpbitWebSocketAdaptor()
    ...
    def tearDown(self) -> None:
        self.ev_loop.run_until_complete(self.websocket.close())
    ...
    @patch("hummingbot.connector.exchange.upbit.upbit_websocket_adaptor.UpbitWebSocketAdaptor._sleep")
    ...
    def test_connect_authenticate_is_called(self, _, ws_connect_mock):
        ...
        self.assertEqual(UpbitWebSocketAdaptor.AUTH_REQUEST, sent_payloads[0]["method"])

    def test_disconnect(self):
        ws = AsyncMock()
        self.websocket._websocket = ws

        self.async_run_with_timeout(self.websocket.close())
    ...
    def test_iter_messages_handle_ping(self, _, ws_connect_mock):
        ...
        expected_pong_payload = {"status": "UP"}
        self.mocking_assistant.add_websocket_text_message(ws_connect_mock.return_value, "PING")
        ...
        sent_payloads = self.mocking_assistant.text_messages_sent_through_websocket(ws_connect_mock.return_value)
        print(sent_payloads)
