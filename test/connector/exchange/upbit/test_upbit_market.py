...
from test.connector.exchange.bittrex.test_bittrex_market import API_MOCK_ENABLED; sys.path.insert(0, realpath(join(__file__, "../../../../../")))
...
from test.connector.exchange.upbit.fixture_upbit import FixtureUpbit
...
API_KEY = "XXX" if API_MOCK_ENABLED else conf.upbit_api_key
API_SECRET = "YYY" if API_MOCK_ENABLED else conf.upbit_secret_key
API_BASE_URL = "api.upbit.com"
EXCHANGE_ORDER_ID = 20001


class UpbitExchangeUnitTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        if API_MOCK_ENABLED:
            ...
            cls.web_app.add_host_to_mock(API_BASE_URL, ["/v1/ticker", "/v1/orderbook",
                                                        "/v1/market/all"])
            cls.web_app.start()
            ...
            cls.web_app.update_response("get", API_BASE_URL, "/v1/accounts", FixtureUpbit.BALANCES)

            cls._t_nonce_patcher = unittest.mock.patch(
                "hummingbot.connector.exchange.upbit.upbit_exchange.get_tracking_nonce")
            cls._t_nonce_mock = cls._t_nonce_patcher.start()
            cls._exch_order_id = 20001
        cls.clock: Clock = Clock(ClockMode.REALTIME)
        print("Set Up Class - First Market")
        cls.market: UpbitExchange = UpbitExchange(
            upbit_api_key=API_KEY,
            upbit_secret_key=API_SECRET,
            trading_pairs=["ETH-KRW"]
        )
        print("Set Up Class - Second Market")
        # Need 2nd instance of market to prevent events mixing up across tests
        cls.market_2: UpbitExchange = UpbitExchange(
            upbit_api_key=API_KEY,
            upbit_secret_key=API_SECRET,
            trading_pairs=["BTC-KRW"]
        )
        print("Initializing Upbit market... this will take about a minute. ")
        print(f"cls.market - {cls.market}")
        print(f"cls.clock - {cls.clock}")
        cls.clock.add_iterator(cls.market)
        cls.clock.add_iterator(cls.market_2)
        print(f"Clock iterator Added")
        cls.stack = contextlib.ExitStack()
        cls._clock = cls.stack.enter_context(cls.clock)
        cls.ev_loop.run_until_complete(cls.wait_til_ready())
        print("Upbit market Ready to test...")
    ...
    async def wait_til_ready(cls):
        print("Wait Til Ready")
        while True:
            now = time.time()
            next_iteration = now // 1.0 + 1
            if cls.market.ready and cls.market_2.ready:
                print("Wait Til Ready - All Ready")
                break
            else:
    ...
    def setUp(self):
        print("Set Up - DB")
        self.db_path: str = realpath(join(__file__, "../upbit_test.sqlite"))
        try:
    ...
    def test_get_fee(self):
        print("Test Get Fee")
        limit_fee: TradeFee = self.market.get_fee("ETH", "KRW", OrderType.LIMIT_MAKER, TradeType.BUY, 1, 10)
        self.assertGreater(limit_fee.percent, 0)
        self.assertEqual(len(limit_fee.flat_fees), 0)
        market_fee: TradeFee = self.market.get_fee("ETH", "KRW", OrderType.LIMIT, TradeType.BUY, 1)
        self.assertGreater(market_fee.percent, 0)
        self.assertEqual(len(market_fee.flat_fees), 0)
        sell_trade_fee: TradeFee = self.market.get_fee("ETH", "KRW", OrderType.LIMIT_MAKER, TradeType.SELL, 1, 10)
    ...
    def place_order(self, is_buy, trading_pair, amount, order_type, price, nonce, post_resp, get_resp):
        ...
        if API_MOCK_ENABLED:
            ...
            self.web_app.update_response("get", API_BASE_URL, f"/v1/orders/{exch_order_id}", resp)
        return order_id, exch_order_id

    def test_fee_overrides_config(self):
        fee_overrides_config_map["upbit_taker_fee"].value = None
        ...
        fee_overrides_config_map["upbit_taker_fee"].value = Decimal('0.2')
        taker_fee: TradeFee = self.market.get_fee("LINK", "ETH", OrderType.LIMIT, TradeType.BUY, Decimal(1),
                                                  Decimal('0.1'))
        self.assertAlmostEqual(Decimal("0.002"), taker_fee.percent)
        fee_overrides_config_map["upbit_maker_fee"].value = None
        ...
        fee_overrides_config_map["upbit_maker_fee"].value = Decimal('0.5')
        maker_fee: TradeFee = self.market.get_fee("LINK", "ETH", OrderType.LIMIT_MAKER, TradeType.BUY, Decimal(1),
                                                  Decimal('0.1'))
    ...
    def test_limit_makers_unfilled(self):
        ...
        order_id, _ = self.place_order(True, trading_pair, quantized_bid_amount, OrderType.LIMIT_MAKER,
                                       ...
                                       FixtureUpbit.ORDER_PLACE, FixtureUpbit.ORDER_GET_BUY_UNMATCHED)
        ...
        order_id, _ = self.place_order(False, trading_pair, quatized_ask_amount, OrderType.LIMIT_MAKER,
                                       ...
                                       FixtureUpbit.ORDER_PLACE, FixtureUpbit.ORDER_GET_SELL_UNMATCHED)
    ...
    def test_limit_taker_buy(self):
        ...
        order_id, _ = self.place_order(True, trading_pair, quantized_amount, OrderType.LIMIT, price, 10001,
                                       FixtureUpbit.ORDER_PLACE, FixtureUpbit.BUY_MARKET_ORDER)
        ...
        self.assertEqual("KRW", buy_order_completed_event.quote_asset)
    ...
    def test_limit_taker_sell(self):
        ...
        order_id, _ = self.place_order(False, trading_pair, amount, OrderType.LIMIT, price, 10001,
                                       FixtureUpbit.ORDER_PLACE, FixtureUpbit.SELL_MARKET_ORDER)
        ...
        self.assertEqual("KRW", sell_order_completed_event.quote_asset)
    ...
    def test_cancel(self):
        ...
        order_id, exch_order_id = self.place_order(False, trading_pair, quantized_amount, OrderType.LIMIT_MAKER,
                                                   ...
                                                   FixtureUpbit.ORDER_PLACE_2, FixtureUpbit.OPEN_SELL_LIMIT_ORDER)
        [order_created_event] = self.run_parallel(self.market_logger.wait_for(SellOrderCreatedEvent))
        if API_MOCK_ENABLED:
            resp = FixtureUpbit.CANCEL_ORDER.copy()
            resp["data"]["cancelledOrderIds"] = [exch_order_id]
            self.web_app.update_response("delete", API_BASE_URL, f"/v1/orders/{exch_order_id}", resp)
        self.market.cancel(trading_pair, order_id)
        if API_MOCK_ENABLED:
            resp = FixtureUpbit.GET_CANCELLED_ORDER.copy()
            resp["data"]["id"] = exch_order_id
            resp["data"]["clientOid"] = order_id
            self.web_app.update_response("get", API_BASE_URL, f"/v1/orders/{exch_order_id}", resp)
    ...
    def test_cancel_all(self):
        ...
        _, exch_order_id = self.place_order(True, trading_pair, quantized_amount, OrderType.LIMIT_MAKER, quantize_bid_price,
                                            10001, FixtureUpbit.ORDER_PLACE, FixtureUpbit.OPEN_BUY_LIMIT_ORDER)

        _, exch_order_id2 = self.place_order(False, trading_pair, quantized_amount, OrderType.LIMIT_MAKER, quantize_ask_price,
                                             10002, FixtureUpbit.ORDER_PLACE, FixtureUpbit.OPEN_SELL_LIMIT_ORDER)

        self.run_parallel(asyncio.sleep(1))
        if API_MOCK_ENABLED:
            resp = FixtureUpbit.ORDERS_BATCH_CANCELLED.copy()
            resp["data"]["cancelledOrderIds"] = [exch_order_id, exch_order_id2]
            self.web_app.update_response("delete", API_BASE_URL, "/v1/orders", resp)
    ...
    def test_orders_saving_and_restoration(self):
        ...
        try:
            ...
            order_id, exch_order_id = self.place_order(True, trading_pair, quantized_amount, OrderType.LIMIT_MAKER,
                                                       quantize_bid_price,
                                                       10001, FixtureUpbit.ORDER_PLACE,
                                                       FixtureUpbit.OPEN_BUY_LIMIT_ORDER)
            [order_created_event] = self.run_parallel(self.market_logger.wait_for(BuyOrderCreatedEvent))
            order_created_event: BuyOrderCreatedEvent = order_created_event
            self.assertEqual(order_id, order_created_event.order_id)
            ...
            for event_tag in self.events:
                self.market.remove_listener(event_tag, self.market_logger)
            self.market: UpbitExchange = UpbitExchange(
                upbit_api_key=API_KEY,
                upbit_secret_key=API_SECRET,
                trading_pairs=["ETH-KRW"]
            )
