...
class UpbitOrderBookTrackerUnitTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.order_book_tracker: UpbitOrderBookTracker = UpbitOrderBookTracker(trading_pairs=cls.trading_pairs)
    ...
    def test_tracker_integrity(self):
        ...
        eth_usdt: OrderBook = order_books["ETH-KRW"]
    ...
    def test_api_get_last_traded_prices(self):
        prices = self.ev_loop.run_until_complete(
            UpbitAPIOrderBookDataSource.get_last_traded_prices(["BTC-KRW", "ETH-BTC"]))
        for key, value in prices.items():
            print(f"{key} last_trade_price: {value}")
        self.assertGreater(prices["BTC-KRW"], 1000)
        self.assertLess(prices["ETH-BTC"], 1)
