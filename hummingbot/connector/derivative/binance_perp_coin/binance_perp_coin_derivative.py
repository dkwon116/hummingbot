...
import hummingbot.connector.derivative.binance_perp_coin.binance_perp_coin_utils as utils
import hummingbot.connector.derivative.binance_perp_coin.constants as CONSTANTS
from hummingbot.connector.derivative.binance_perp_coin.binance_perp_coin_in_flight_order import (
    BinancePerpCoinInFlightOrder
)
...
class BinancePerpCoinDerivative(ExchangeBase, PerpetualTrading):
    ...
    def __init__(self,
                 ...
                 domain: str = "binance_perp_coin",
                 ):
        self._api_key = binance_perp_coin_api_key
        self._api_secret = binance_perp_coin_api_secret
        ...
        self._order_book_tracker = BinancePerpCoinOrderBookTracker(trading_pairs=trading_pairs, domain=self._domain, throttler=self._throttler)
    ...
    @property
    def name(self) -> str:
        # Note: domain here refers to the entire exchange name. i.e. binance_perp_coin or binance_perp_coin_testnet
        return self._domain
    ...
    @property
    def in_flight_orders(self) -> Dict[str, BinancePerpCoinInFlightOrder]:
    ...
    async def create_order(self,
        ...
        order_result = None
        api_params = {"symbol": utils.convert_to_exchange_trading_pair(trading_pair),
        ...
        try:
            order_result = await self.request(path=CONSTANTS.ORDER_URL,
                                              params=api_params,
            ...
            self.logger().info(f"Created {order_type.name.lower()} {trade_type.name.lower()} order {order_id} for "
                               f"{amount} Cont {(amount * trading_rule.min_notional_size)} {trading_pair}.")
    ...
    async def execute_cancel(self, trading_pair: str, client_order_id: str) -> Dict[str, Any]:
        try:
            ...
            tracked_order: Optional[BinancePerpCoinInFlightOrder] = self._in_flight_orders.get(client_order_id, None)
    ...
    def start_tracking_order(self, order_id: str, exchange_order_id: str, trading_pair: str, trading_type: object,
        ...
        self._in_flight_orders[order_id] = BinancePerpCoinInFlightOrder(
    ...
    def get_fee(self, base_currency: str, quote_currency: str, order_type: object, order_side: object,
        ...
        return estimate_fee("binance_perp_coin", is_maker)
    ...
    async def _update_trading_rules(self):
        ...
        if current_tick > last_tick or len(self._trading_rules) < 1:
            exchange_info = await self.request(path=CONSTANTS.EXCHANGE_INFO_URL, method=MethodType.GET, is_signed=False)
    ...
    async def _update_positions(self):
        positions = await self.request(path=CONSTANTS.POSITION_INFORMATION_URL,
                                       add_timestamp=True,
                                       is_signed=True)
        for position in positions:
    ...
    async def _update_order_status(self):
        ...
        if current_tick > last_tick and len(self._in_flight_orders) > 0:
            ...
            tasks = [self.request(path=CONSTANTS.ORDER_URL,
                                  params={
            ...
            for order_update, tracked_order in zip(results, tracked_orders):
                ...
                executed_amount_base = Decimal(order_update.get("cumBase", "0"))
                executed_amount_quote = Decimal(order_update.get("cumBase", "0"))
    ...
    async def get_funding_payment(self, startTime):
        ...
        for pair in self._trading_pairs:
            funding_payment_tasks.append(self.request(path=CONSTANTS.GET_INCOME_HISTORY_URL,
                                                      params={"symbol": utils.convert_to_exchange_trading_pair(pair), "incomeType": "FUNDING_FEE", "startTime": int(startTime * 1000)},
                                                      method=MethodType.GET,
                                                      add_timestamp=True,
                                                      is_signed=True))
