...
from hummingbot.connector.derivative.binance_perp_coin import binance_perp_coin_utils as utils
from hummingbot.connector.derivative.binance_perp_coin.binance_perp_coin_order_book import BinancePerpCoinOrderBook
from hummingbot.connector.derivative.binance_perp_coin.binance_perp_coin_utils import convert_to_exchange_trading_pair
import hummingbot.connector.derivative.binance_perp_coin.constants as CONSTANTS
from hummingbot.logger import HummingbotLogger


class BinancePerpCoinAPIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    async def fetch_trading_pairs(domain: str = CONSTANTS.DOMAIN, throttler: Optional[AsyncThrottler] = None) -> List[str]:
        ...
        throttler = throttler or BinancePerpCoinAPIOrderBookDataSource._get_throttler_instance()
            ...
            async with aiohttp.ClientSession() as client:
                async with client.get(url=url, timeout=10) as response:
                    if response.status == 200:
                        ...
                        raw_trading_pairs = [d["symbol"] for d in data["symbols"] if d["contractStatus"] == "TRADING"]
                        trading_pair_targets = [
                            f"{d['baseAsset']}-{d['quoteAsset']}" for d in data["symbols"] if d["contractStatus"] == "TRADING"
                        ]
    ...
    async def get_new_order_book(self, trading_pair: str) -> OrderBook:
        ...
        snapshot_msg: OrderBookMessage = BinancePerpCoinOrderBook.snapshot_message_from_exchange(
        ...
        while True:
            try:
                ...
                async for json_msg in self._iter_messages(ws):
                    ...
                    order_book_message: OrderBookMessage = BinancePerpCoinOrderBook.diff_message_from_exchange(
                        json_msg, timestamp
                    )
        ...
        while True:
            try:
                ...
                async for json_msg in self._iter_messages(ws):
                    if "result" in json_msg:
                        continue
                    trade_msg: OrderBookMessage = BinancePerpCoinOrderBook.trade_message_from_exchange(json_msg)
