...
class FtxPerpAPIUserStreamDataSource(UserStreamTrackerDataSource):
    ...
    def __init__(self, ftx_perp_auth: FtxPerpAuth):
        super().__init__()
    ...
    async def set_subscriptions(self, ws: websockets.WebSocketClientProtocol):
        await ws.send(json.dumps(self._ftx_perp_auth.generate_websocket_subscription()))
            ...
            try:
                ...
                async for message in self._inner_messages(ws):
                    ...
                    if decoded['type'] == 'error':
                        self.logger().warning(f"Error returned from ftx_perp user stream: {decoded['code']}:{decoded['msg']}")
