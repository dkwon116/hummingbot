...
from hummingbot.connector.derivative.ftx_perp.ftx_perp_active_order_tracker import FtxPerpActiveOrderTracker
...
from hummingbot.connector.derivative.ftx_perp.ftx_perp_order_book_message import FtxPerpOrderBookMessage
from hummingbot.connector.derivative.ftx_perp.ftx_perp_order_book_tracker_entry import FtxPerpOrderBookTrackerEntry
...
class FtxPerpOrderBookTracker(OrderBookTracker):
    ...
    def __init__(self,
        ...
        self._order_books: Dict[str, FtxPerpOrderBook] = {}
        self._saved_message_queues: Dict[str, Deque[FtxPerpOrderBookMessage]] = defaultdict(lambda: deque(maxlen=1000))
        self._active_order_trackers: Dict[str, FtxPerpActiveOrderTracker] = defaultdict(FtxPerpActiveOrderTracker)
    ...
    async def _refresh_tracking_tasks(self):
        ...
        available_pairs: Dict[str, FtxPerpOrderBookTrackerEntry] = await self.data_source.get_tracking_pairs()
        ...
        for trading_pair in new_trading_pair:
            order_book_tracker_entry: FtxPerpOrderBookTrackerEntry = available_pairs[trading_pair]
