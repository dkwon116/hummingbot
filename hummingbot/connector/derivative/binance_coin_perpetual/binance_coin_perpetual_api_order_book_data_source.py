...
import hummingbot.connector.derivative.binance_coin_perpetual.binance_coin_perpetual_utils as utils
import hummingbot.connector.derivative.binance_coin_perpetual.binance_coin_perpetual_web_utils as web_utils
import hummingbot.connector.derivative.binance_coin_perpetual.constants as CONSTANTS
from hummingbot.connector.derivative.binance_coin_perpetual.binance_coin_perpetual_order_book import BinanceCoinPerpetualOrderBook
...
class BinanceCoinPerpetualAPIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    async def fetch_trading_pairs(
        ...
        symbols_map = await BinanceCoinPerpetualAPIOrderBookDataSource.trading_pair_symbol_map(
            domain=domain,
            throttler=throttler,
            api_factory=api_factory,
            time_synchronizer=time_synchronizer)
    ...
    async def get_new_order_book(self, trading_pair: str) -> OrderBook:
        ...
        snapshot_msg: OrderBookMessage = BinanceCoinPerpetualOrderBook.snapshot_message_from_exchange(
    ...
    async def listen_for_order_book_diffs(self, ev_loop: asyncio.BaseEventLoop, output: asyncio.Queue):
        while True:
            ...
            msg.data["data"]["s"] = await self.convert_from_exchange_trading_pair(
                ...
                api_factory=self._api_factory,
                time_synchronizer=self._time_synchronizer)
            order_book_message: OrderBookMessage = BinanceCoinPerpetualOrderBook.diff_message_from_exchange(
                msg.data, timestamp
            )
    ...
    async def listen_for_trades(self, ev_loop: asyncio.BaseEventLoop, output: asyncio.Queue):
        while True:
            ...
            msg.data["data"]["s"] = await self.convert_from_exchange_trading_pair(
                ...
                api_factory=self._api_factory,
                time_synchronizer=self._time_synchronizer)
            trade_message: OrderBookMessage = BinanceCoinPerpetualOrderBook.trade_message_from_exchange(msg.data)
