...
from hummingbot.connector.derivative.binance_coin_perpetual import (
    binance_coin_perpetual_web_utils as web_utils,
    constants as CONSTANTS,
    binance_coin_perpetual_utils as utils,
)
from hummingbot.connector.derivative.binance_coin_perpetual.binance_coin_perpetual_api_order_book_data_source import (
    BinanceCoinPerpetualAPIOrderBookDataSource,
)
from hummingbot.connector.derivative.binance_coin_perpetual.binance_coin_perpetual_auth import BinanceCoinPerpetualAuth
from hummingbot.connector.derivative.binance_coin_perpetual.binance_coin_perpetual_order_book_tracker import (
    BinanceCoinPerpetualOrderBookTracker,
)
from hummingbot.connector.derivative.binance_coin_perpetual.binance_coin_perpetual_user_stream_data_source import (
    BinanceCoinPerpetualUserStreamDataSource,
)
...
class BinanceCoinPerpetualDerivative(ExchangeBase, PerpetualTrading):
    ...
    def __init__(
            ...
            binance_coin_perpetual_api_key: str = None,
            binance_coin_perpetual_api_secret: str = None,
            trading_pairs: Optional[List[str]] = None,
            trading_required: bool = True,
            domain: str = CONSTANTS.DOMAIN,
    ):
        self._binance_time_synchronizer = TimeSynchronizer()
        self._auth: BinanceCoinPerpetualAuth = BinanceCoinPerpetualAuth(api_key=binance_coin_perpetual_api_key,
                                                                        api_secret=binance_coin_perpetual_api_secret,
        ...
        self._user_stream_tracker = UserStreamTracker(
            data_source=BinanceCoinPerpetualUserStreamDataSource(
                auth=self._auth,
                domain=self._domain,
                throttler=self._throttler,
                api_factory=self._api_factory,
                time_synchronizer=self._binance_time_synchronizer))
        self._set_order_book_tracker(BinanceCoinPerpetualOrderBookTracker(
    ...
    @property
    def name(self) -> str:
        # Note: domain here refers to the entire exchange name. i.e. binance_coin_perpetual or binance_coin_perpetual_testnet
        return self._domain
    ...
    def status_dict(self):
        sd = {
            "symbols_mapping_initialized": BinanceCoinPerpetualAPIOrderBookDataSource.trading_pair_symbol_map_ready(
                domain=self._domain),
    ...
    async def cancel_all(self, timeout_seconds: float):
        ...
        try:
            ...
            self.logger().network(
                ...
                exc_info=True,
                app_warning_msg="Failed to cancel order with Binance Coin Perpetual. Check API key and network connection."
            )
    ...
    def quantize_order_value(self, trading_pair, value):
        trading_rule: TradingRule = self._trading_rules[trading_pair]
        return floor(value / trading_rule.min_notional_size) * trading_rule.min_notional_size

    def get_contract_amount_from_value(self, trading_pair, value):
        trading_rule: TradingRule = self._trading_rules[trading_pair]
        return round(value / trading_rule.min_notional_size)

    def get_value_from_contract(self, trading_pair, contract):
        trading_rule: TradingRule = self._trading_rules[trading_pair]
        return contract * trading_rule.min_notional_size
    ...
    def _format_trading_rules(self, exchange_info_dict: Dict[str, Any]) -> List[TradingRule]:
        ...
        for rule in rules:
            try:
                if rule["contractType"] == "PERPETUAL" and rule["contractStatus"] == "TRADING":
        ...
        while True:
            try:
                ...
                self.logger().network(
                    ...
                    exc_info=True,
                    app_warning_msg="Could not fetch new trading rules from Binance Coin Perpetuals. "
    ...
    async def _fetch_funding_payment(self, trading_pair: str) -> bool:
        ...
        try:
            response = await self._api_request(
                ...
                params={
                    "symbol": await BinanceCoinPerpetualAPIOrderBookDataSource.convert_to_exchange_trading_pair(
            ...
            for funding_payment in response:
                ...
                trading_pair = await BinanceCoinPerpetualAPIOrderBookDataSource.convert_from_exchange_trading_pair(
        ...
        while True:
            try:
                ...
                self.logger().network("Unexpected error while fetching account updates.", exc_info=True,
                                      app_warning_msg="Could not fetch account updates from Binance Coin Perpetuals. "
        ...
        if current_tick > last_tick and len(self._client_order_tracker.active_orders) > 0:
            ...
            tasks = [
                self._api_request(
                    ...
                    params={"symbol": await BinanceCoinPerpetualAPIOrderBookDataSource.convert_to_exchange_trading_pair(
                        hb_trading_pair=trading_pair,
                        domain=self._domain,
                        throttler=self._throttler,
                        api_factory=self._api_factory,
    ...
    async def _create_order(
        ...
        try:
            ...
            self.logger().network(
                f"Error submitting order to Binance Coin Perpetuals for {amount} {trading_pair} "
    ...
    async def _execute_cancel(self, trading_pair: str, client_order_id: str) -> str:
        ...
        try:
            ...
            if response.get("code") == -2011 or "Unknown order sent" in response.get("msg", ""):
                self.logger().debug(f"The order {client_order_id} does not exist on Binance Coin Perpetuals. "
    ...
    async def trading_pair_symbol_map(self):
        ...
        return await BinanceCoinPerpetualAPIOrderBookDataSource.trading_pair_symbol_map(
            domain=self._domain,
            throttler=self._throttler,
            api_factory=self._api_factory,
            time_synchronizer=self._binance_time_synchronizer)
    ...
    async def get_last_traded_prices(self, trading_pairs: List[str]) -> Dict[str, float]:
        ...
        return await BinanceCoinPerpetualAPIOrderBookDataSource.get_last_traded_prices(
            trading_pairs=trading_pairs,
            domain=self._domain
        )
