...
from hummingbot.connector.exchange.gopax.gopax_order_book import GopaxOrderBook
from hummingbot.connector.exchange.gopax.gopax_order_book_message import GopaxOrderBookMessage
from hummingbot.connector.exchange.gopax.gopax_utils import split_orderbook_to_bidask, split_orderbook_to_bidask_simple
...
class GopaxAPIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    async def get_last_traded_prices(
        ...
        for trading_pair in trading_pairs:
            if trading_pair in cls._last_traded_prices:
                ...
                pairs_to_update.append(trading_pair)

        throttler = throttler or cls._get_throttler_instance()
        async with throttler.execute_task(CONSTANTS.GET_TICKER_URL):
            async with shared_client.get(
                f"{gopax_utils.rest_api_url(domain) + CONSTANTS.GET_TICKER_URL}"
            ) as response:
                if response.status == 200:
                    resp_json = await response.json()
                    results = {pair["name"]: float(pair["close"]) for pair in resp_json if pair["name"] in pairs_to_update}
        return results
    ...
    async def fetch_trading_pairs(domain: str = None, throttler: Optional[AsyncThrottler] = None) -> List[str]:
        ...
        async with aiohttp.ClientSession() as client:
            throttler = throttler or GopaxAPIOrderBookDataSource._get_throttler_instance()
            async with throttler.execute_task(CONSTANTS.GET_MARKETS_URL):
                async with client.get(
                    f"{gopax_utils.rest_api_url(domain) + CONSTANTS.GET_MARKETS_URL}"
                ) as response:
                    if response.status == 200:
                        resp_json = await response.json()
                        return [pair['name'] for pair in resp_json]
                    return []
    ...
    async def get_snapshot(self, trading_pair: str, domain: Optional[str] = None, throttler: Optional[AsyncThrottler] = None) -> Dict[str, any]:
        ...
        async with throttler.execute_task(CONSTANTS.GET_ORDER_BOOK_URL):
            url = f"{gopax_utils.rest_api_url(domain) + CONSTANTS.GET_ORDER_BOOK_URL + trading_pair}/book"
            async with self._shared_client.get(url) as response:
                ...
                data = split_orderbook_to_bidask(response_ls)
                data["timestamp"] = ts
                return data
    ...
    async def get_new_order_book(self, trading_pair: str) -> OrderBook:
        ...
        snapshot_msg: GopaxOrderBookMessage = GopaxOrderBook.snapshot_message_from_exchange(
            msg=snapshot,
    ...
    async def _create_websocket_connection(self) -> GopaxWebSocketAdaptor:
        """
        Initialize WebSocket client for APIOrderBookDataSource
        """
        try:
            ws = GopaxWebSocketAdaptor(shared_client=self._get_session_instance(), auth_assistant=self._auth_assistant)
            await ws.connect()
            return ws
            ...
            try:
                ...
                async for msg in ws.iter_messages():
                    ...
                    if "errCode" in msg[GopaxWebSocketAdaptor._msg_content_field_name]:
                        raise Exception()
                    if GopaxWebSocketAdaptor._msg_type_field_name not in msg:
                        continue
                    channel = msg[GopaxWebSocketAdaptor._msg_type_field_name]
                    if channel in GopaxWebSocketAdaptor.SUBSCRIPTION_LIST:
                        self._message_queue[channel].put_nowait(msg[GopaxWebSocketAdaptor._msg_content_field_name])
            except asyncio.CancelledError:
                raise
            except Exception:
                self.logger().error(
    ...
    async def listen_for_trades(self, ev_loop: asyncio.BaseEventLoop, output: asyncio.Queue):
        ...
        """
        msg_queue = self._message_queue[GopaxWebSocketAdaptor.TRADE_EVENT_NAME]
        while True:
            try:
                payload = await msg_queue.get()

                trade_timestamp: int = payload["occurredAt"] / 1000
                trade_msg: GopaxOrderBookMessage = GopaxOrderBook.trade_message_from_exchange(
                    payload,
                    trade_timestamp,
                    metadata={"trading_pair": payload["tradingPairName"]},
                )
    ...
    async def listen_for_order_book_diffs(self, ev_loop: asyncio.BaseEventLoop, output: asyncio.Queue):
        ...
        """
        msg_queue = self._message_queue[GopaxWebSocketAdaptor.ORDERBOOK_EVENT_NAME]
        while True:
            try:
                payload = await msg_queue.get()
                timestamp = time.time() * 1000
                trading_pair = payload["tradingPairName"]
                order_book_data = split_orderbook_to_bidask_simple(payload, timestamp)
                ...
                # Gopax does not offer order book diff ws updates.
                orderbook_msg: GopaxOrderBookMessage = GopaxOrderBook.diff_message_from_exchange(
                    order_book_data,
                    timestamp,
                    metadata={"trading_pair": trading_pair},
                )
        ...
        while True:
            try:
                for trading_pair in self._trading_pairs:
                    ...
                    snapshot_message: GopaxOrderBookMessage = GopaxOrderBook.snapshot_message_from_exchange(
                        msg=snapshot,
                        trading_pair=trading_pair,
