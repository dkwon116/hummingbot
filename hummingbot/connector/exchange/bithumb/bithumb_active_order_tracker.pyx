...
BithumbOrderBookTrackingDictionary = Dict[Decimal, Dict[str, Dict[str, any]]]

cdef class BithumbActiveOrderTracker:
    def __init__(self,
                 active_asks: BithumbOrderBookTrackingDictionary = None,
                 active_bids: BithumbOrderBookTrackingDictionary = None):
        super().__init__()
    ...
    @property
    def active_asks(self) -> BithumbOrderBookTrackingDictionary:
        return self._active_asks

    @property
    def active_bids(self) -> BithumbOrderBookTrackingDictionary:
    ...
    def get_rates_and_quantities(self, entry) -> tuple:
        ...
        print(f"get_rates-{price}")
        return price, Decimal(entry[1])
    ...
    cdef tuple c_convert_snapshot_message_to_np_arrays(self, object message):
        ...
        cdef:
            bid_array = [[message.timestamp, float(price), sum([order_dict["amount"] for order_dict in self._active_bids[price].values()]), message.update_id] 
                        for price in sorted(list(self._active_bids.keys()), reverse=True)]
            ask_array = [[message.timestamp, float(price), sum([order_dict["amount"] for order_dict in self._active_asks[price].values()]), message.update_id] 
                        for price in sorted(list(self._active_asks.keys()), reverse=True)]
            np.ndarray[np.float64_t, ndim=2] bids = np.array(bid_array, dtype="float64", ndmin=2)
            np.ndarray[np.float64_t, ndim=2] asks = np.array(ask_array, dtype="float64", ndmin=2)
