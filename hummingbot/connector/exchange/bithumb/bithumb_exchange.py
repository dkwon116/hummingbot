...
class BithumbExchange(ExchangeBase):
    """
    BithumbExchange connects with Crypto.com exchange and provides order book pricing, user account tracking and
    ...
    def __init__(self,
        ...
        """
        :param bithumb_api_key: The API key to connect to private Crypto.com APIs.
        :param bithumb_secret_key: The API secret.
        ...
        self._in_flight_orders = {}  # Dict[client_order_id:str, BithumbInFlightOrder]
    ...
    @property
    def in_flight_orders(self) -> Dict[str, BithumbInFlightOrder]:
    ...
    def restore_tracking_states(self, saved_states: Dict[str, any]):
        ...
        self._in_flight_orders.update({
            key: BithumbInFlightOrder.from_json(value)
        ...
        while True:
            try:
                ...
                self.logger().network(f"Unexpected error while fetching trading rules. Error: {str(e)}",
                                      exc_info=True,
                                      app_warning_msg="Could not fetch new trading rules from Bithumb. "
                                                      "Check network connection.")
                await asyncio.sleep(0.5)

    async def _update_trading_rules(self):
        """
        Bithumb does not give all market trading rule in single call.
    ...
    def _format_trading_rules(self, markets: Dict[str, Any]) -> Dict[str, TradingRule]:
        ...
        for trading_pair in markets:
            try:
                ...
                price_step = Decimal(str(bithumb_utils.get_price_increments(current_price)))
                quantity_step = Decimal("1") / Decimal(str(math.pow(10, 4)))
                min_order_size = Decimal(str(bithumb_utils.get_min_order_size(current_price) * 1.1))
                min_order_value = Decimal("501")
    ...
    async def _api_request(self,
        ...
        async with self._throttler.execute_task(path_url):
            base_url = bithumb_utils.rest_api_url(None)
            url = base_url + path_url
            client = await self._http_client()
            if is_auth_required:
                headers = self._bithumb_auth.get_auth_headers(path_url, params)
            else:
            ...
            if response.status not in [200, 201]:
                raise self.logger().warning(f"Error fetching data from {url}. HTTP status is {response.status}. "
    ...
    def get_order_price_quantum(self, trading_pair: str, price: Decimal):
        ...
        return Decimal(str(bithumb_utils.get_price_increments(price)))
    ...
    def buy(self, trading_pair: str, amount: Decimal, order_type=OrderType.MARKET,
        ...
        """
        order_id: str = bithumb_utils.get_new_client_order_id(True, trading_pair)
    ...
    def sell(self, trading_pair: str, amount: Decimal, order_type=OrderType.MARKET,
        ...
        :returns A new internal order id
        """
        order_id: str = bithumb_utils.get_new_client_order_id(False, trading_pair)
    ...
    async def _create_order(self,
        ...
        if order_type.is_limit_type():
            api_params = {"order_currency": trading_pair.split("-")[0],
                          "payment_currency": trading_pair.split("-")[1],
                          "type": "bid" if trade_type.name == "BUY" else "ask",
                          "price": f"{price:f}",
                          "units": f"{amount:f}"
                          }
        ...
        try:
            if order_type.is_limit_type():
                order_result = await self._api_request("post", CONSTANTS.CREATE_LIMIT_ORDER_PATH_URL, api_params, True)
            elif order_type is OrderType.MARKET:
                if trade_type.name == "BUY":
                    order_result = await self._api_request("post", CONSTANTS.CREATE_MARKET_BUY_ORDER_PATH_URL, api_params, True)
                elif trade_type.name == "SELL":
                    order_result = await self._api_request("post", CONSTANTS.CREATE_MARKET_SELL_ORDER_PATH_URL, api_params, True)
            ...
            self.logger().network(
                f"Error submitting {trade_type.name} {order_type.name} order to Bithumb for "
                f"{amount} {trading_pair} "
                f"{price}.",
                exc_info=True,
    ...
    def start_tracking_order(self,
        ...
        """
        self._in_flight_orders[order_id] = BithumbInFlightOrder(
    ...
    async def _execute_cancel(self, trading_pair: str, order_id: str) -> str:
        ...
        try:
            ...
            api_params = {
                ...
                "type": "bid" if tracked_order.trade_type.name == "BUY" else "ask",
                "order_currency": tracked_order.base_asset,
                "payment_currency": tracked_order.quote_asset
            }
            await self._api_request("post", CONSTANTS.CANCEL_ORDER_PATH_URL, api_params, True)
            return order_id
        except asyncio.CancelledError:
            raise
        except Exception as e:
            self.logger().network(
                ...
                exc_info=True,
                app_warning_msg=f"Failed to cancel the order {order_id} on Bithumb. "
    ...
    async def _status_polling_loop(self):
        ...
        Bithumb does not have User Stream rely only on REST API
        """
        while True:
            try:
                self._poll_notifier = asyncio.Event()
    ...
    async def _update_balances(self):
        ...
        account_info = await self._api_request("post", CONSTANTS.GET_ACCOUNT_SUMMARY_PATH_URL, {"currency": "ALL"}, True)

        assets = account_info["data"]
        asset_list = [asset.split("_")[1] for asset in assets if asset.split("_")[0] == "total" and float(assets[asset]) > 0]

        for asset in asset_list:
            asset_name = asset.upper()
            self._account_available_balances[asset_name] = Decimal(assets["available_"+ asset])
            self._account_balances[asset_name] = Decimal(assets["total_"+ asset])
    ...
    async def _update_order_status(self):
        ...
        if current_tick > last_tick and len(self._in_flight_orders) > 0:
            ...
            for tracked_order in tracked_orders:
                ...
                tasks.append(self._api_request("post", CONSTANTS.GET_ORDER_DETAIL_PATH_URL, api_params, True))
                orders.append((order_id, tracked_order.price))
            ...
            for response in responses:
                ...
                if float(result["order_price"]) == float(orders[rsp_no][1]):
                    result["uuid"] = orders[rsp_no][0]
                rsp_no += 1
                if "uuid" not in result:
                    self.logger().info(f"_update_order_status result not in resp: {response}")
                    continue

                if len(result["contract"]) > 0:
                    for trade_msg in result["contract"]:
                        trade_msg["order_uuid"] = result["uuid"]
                        trade_msg["paid_fee"] = trade_msg["fee"]
    ...
    async def _process_trade_message(self, trade_msg: Dict[str, Any]):
        ...
        fee_paid = Decimal(str(trade_msg["paid_fee"]))
        ...
        if math.isclose(tracked_order.executed_amount_base, tracked_order.amount) or \
                tracked_order.executed_amount_base >= tracked_order.amount or tracked_order.last_state == "Completed":
    ...
    async def cancel_all(self, timeout_seconds: float):
        ...
        pairs = list(set([o.trading_pair for o in incomplete_orders]))
        ...
        try:
            ...
            open_orders = await self.get_open_orders(pairs)
            ...
            self.logger().network(
                ...
                app_warning_msg="Failed to cancel order on Bithumb. Check API key and network connection."
            )
    ...
    async def get_open_orders(self, pairs) -> List[OpenOrder]:
        ...
        for pair in pairs:
            ...
            result = await self._api_request("post", CONSTANTS.GET_OPEN_ORDERS_PATH_URL, {"order_currency": currency}, True)
            
            for order in result:
                # if bithumb_utils.HBOT_BROKER_ID not in order["client_oid"]:
                #     continue
                if order["type"] != "LIMIT":
                    raise Exception(f"Unsupported order type {order['type']}")
                ret_val.append(
                    OpenOrder(
                        trading_pair=order["order_currency"]+"-"+order["payment_currency"],
                        price=Decimal(str(order["price"])),
                        amount=Decimal(str(order["units"])),
                        executed_amount=Decimal(str(order["units"]))-Decimal(str(order["units_remaining"])),
                        status="Open",
                        order_type=OrderType.LIMIT,
                        is_buy=True if order["type"].lower() == "bid" else False,
                        time=int(order["order_date"]),
