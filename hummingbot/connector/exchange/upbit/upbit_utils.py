...
def rest_api_url(connector_variant_label: Optional[str]) -> str:
    variant = connector_variant_label if connector_variant_label else "upbit_main"
...
def split_orderbook_to_bidask(orderbook, timestamp):
    update_id: float = timestamp / 1000
    bids = [[unit["bid_price"], unit["bid_size"], update_id] for unit in orderbook]
    asks = [[unit["ask_price"], unit["ask_size"], update_id] for unit in orderbook]
    return {"asks": asks, "bids": bids}


def split_orderbook_to_bidask_simple(orderbook, timestamp):
    update_id: float = timestamp / 1000
    bids = [[unit["bp"], unit["bs"], update_id] for unit in orderbook]
    asks = [[unit["ap"], unit["as"], update_id] for unit in orderbook]
    return {"asks": asks, "bids": bids}
